...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    if [[ $(print -P "%#") == '#' ]]; then
      # Shell runs as root
      "$1_prompt_segment" "$0_ROOT" "$2" "$DEFAULT_COLOR" "green" "$USER@%m"
    else
      "$1_prompt_segment" "$0_DEFAULT" "$2" "$DEFAULT_COLOR" "green" "$USER@%m"
    fi
  fi
}
...
prompt_dir() {
  ...
  local current_icon=''
  if [[ $(print -P "%~") == '~' ]]; then
    "$1_prompt_segment" "$0_HOME" "$2" "$DEFAULT_COLOR" "yellow" "$current_path" 'HOME_ICON'
  elif [[ $(print -P "%~") == '~'* ]]; then
    "$1_prompt_segment" "$0_HOME_SUBFOLDER" "$2" "$DEFAULT_COLOR" "yellow" "$current_path" 'HOME_SUB_ICON'
  else
    "$1_prompt_segment" "$0_DEFAULT" "$2" "$DEFAULT_COLOR" "yellow" "$current_path" 'FOLDER_ICON'
  fi
}
